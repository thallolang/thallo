...
#define IGNORE_DENSE_THRESHOLDS 0
#define IGNORE_TGT_NORMAL_VALIDITY 0
#define IGNORE_TGT_POS_VALIDITY 0
#define IGNORE_INSCREEN_VALIDITY 0
#define IGNORE_SRC_NORMAL_VALIDITY 0
#define IGNORE_SRC_POS_VALIDITY 0
	...
	float depthMin, float depthMax)
{
	unsigned int x = idx % imageWidth;		unsigned int y = idx / imageWidth;
	const float3 cposj = depthToCamera(intrinsics.x, intrinsics.y, intrinsics.z, intrinsics.w, make_int2(x, y), srcDepth[idx]);
    if (IGNORE_SRC_POS_VALIDITY || (cposj.z > depthMin && cposj.z < depthMax)) {
		float3 nrmj = make_float3(srcNormals[idx]);
        if (IGNORE_SRC_NORMAL_VALIDITY || nrmj.x != MINF) {
			nrmj = transform.getFloat3x3() * nrmj;
			float3 camPosSrcToTgt = transform * cposj;
			float2 tgtScreenPosf = cameraToDepth(intrinsics.x, intrinsics.y, intrinsics.z, intrinsics.w, camPosSrcToTgt);
			int2 tgtScreenPos = make_int2((int)roundf(tgtScreenPosf.x), (int)roundf(tgtScreenPosf.y));
            if (IGNORE_INSCREEN_VALIDITY) return true;
			if (tgtScreenPos.x >= 0 && tgtScreenPos.y >= 0 && tgtScreenPos.x < (int)imageWidth && tgtScreenPos.y < (int)imageHeight) {
				float3 camPosTgt = depthToCamera(intrinsics.x, intrinsics.y, intrinsics.z, intrinsics.w, tgtScreenPos, tgtDepth[tgtScreenPos.y * imageWidth + tgtScreenPos.x]);
                if (IGNORE_TGT_POS_VALIDITY || (camPosTgt.z > depthMin && camPosTgt.z < depthMax) ) {
                    ...
                    if (IGNORE_TGT_NORMAL_VALIDITY || normalTgt.x != MINF) {
						float dist = length(camPosSrcToTgt - camPosTgt);
						float dNormal = dot(nrmj, normalTgt);
                        if ((IGNORE_DENSE_THRESHOLDS) || (dNormal >= normalThresh && dist <= distThresh) ) {
							return true;
						}
					}
				}
    ...
    if (IGNORE_SRC_POS_VALIDITY || (cposj.z > depthMin && cposj.z < depthMax)) {
        ...
        if (IGNORE_SRC_NORMAL_VALIDITY || nrmj.x != MINF) {
			...
			if (tgtScreenPos.x >= 0 && tgtScreenPos.y >= 0 && tgtScreenPos.x < (int)imageWidth && tgtScreenPos.y < (int)imageHeight) {
                ...
                if (IGNORE_TGT_POS_VALIDITY || (cposi.z > depthMin && cposi.z < depthMax) ) {
                    ...
                    if (IGNORE_TGT_NORMAL_VALIDITY || nrmi.x != MINF) {
...
#if AVOID_UNDEFINED_WARP_INSTRUCTIONS
            warpReduceAndAtomicAdd(&d_JtJ[(vi * 6 + j)*dim + (vi * 6 + i)], dii);
            warpReduceAndAtomicAdd(&d_JtJ[(vj * 6 + j)*dim + (vj * 6 + i)], djj);
            warpReduceAndAtomicAdd(&d_JtJ[(vj * 6 + j)*dim + (vi * 6 + i)], dij);
            warpReduceAndAtomicAdd(&d_JtJ[(vj * 6 + i)*dim + (vi * 6 + j)], dji);
#else
...
#if AVOID_UNDEFINED_WARP_INSTRUCTIONS
        warpReduceAndAtomicAdd(&d_Jtr[vi * 6 + i], jtri);
        warpReduceAndAtomicAdd(&d_Jtr[vj * 6 + i], jtrj);
#else
...
#   if AVOID_UNDEFINED_WARP_INSTRUCTIONS
        warpReduceAndAtomicAdd(d_sumResidualDEBUG, res);
        warpReduceAndAtomicAdd(d_numCorrDEBUG, num);
#   else
