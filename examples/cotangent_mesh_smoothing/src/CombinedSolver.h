...
public:
    CombinedSolver(const SimpleMesh* mesh, bool performanceRun, CombinedSolverParameters params, float weightFit, float weightReg) : CombinedSolverBase("Cotangent Mesh Smoothing")
    {
        ...
        m_unknown = createEmptyThalloImage(imdims, ThalloImage::Type::FLOAT, 3, ThalloImage::GPU, true);
        m_target = createEmptyThalloImage(imdims, ThalloImage::Type::FLOAT, 3, ThalloImage::GPU, true);
    ...
    void initializeConnectivity() {
        ...
        m_graph = std::make_shared<ThalloGraph>(std::vector<std::vector<int> >({ head, tail, prev, next }));
    }
