...
SimpleBuffer::SimpleBuffer(float* data, int size, bool onGPU) :
m_onGPU(onGPU),
m_width(1),
m_height(1),
m_channelCount(size),
m_dataType(DataType::FLOAT)
{
    ...
    if (onGPU) {
        cudaMalloc(&m_data, dataSize);
        cudaMemcpy(m_data, data, dataSize, cudaMemcpyHostToDevice);
    }
    else {
        m_data = malloc(dataSize);
...
SimpleBuffer::SimpleBuffer(float* data, int width, int height, bool onGPU) :
m_onGPU(onGPU),
m_width(width),
m_height(height),
m_channelCount(1),
