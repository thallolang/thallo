...
public:
    struct Parameter {
        std::string name = "";
        void* ptr = nullptr;
        std::shared_ptr<ThalloImage> im = nullptr;
        std::shared_ptr<ThalloGraph> graph = nullptr;
        Parameter() {}
        Parameter(std::string _name, void* _ptr, std::shared_ptr<ThalloImage> _im = nullptr, std::shared_ptr<ThalloGraph> _gr = nullptr) :
    ...
    void set(const std::string& name, std::shared_ptr<ThalloImage> im) {
        auto location = std::find_if(m_parameters.begin(), m_parameters.end(), [name](const Parameter& p){ return p.name == name; });
        if (location == m_parameters.end()) {
    ...
    void set(const std::string& name, std::shared_ptr<ThalloGraph> graph) {
        auto location = std::find_if(m_parameters.begin(), m_parameters.end(), [name](const Parameter& p){ return p.name == name; });
        if (location == m_parameters.end()) {
