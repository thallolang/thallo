...
public:
    CombinedSolver(const ColorImageR32& sourceImage, const ColorImageR32& targetImage, const CombinedSolverParameters& params) :
		...
		resetGPU();
        addThalloSolvers(m_levels[0].m_dims);
	}
...
private:
	...
	struct HierarchyLevel {
		...
		void init(const ColorImageR32& source, const ColorImageR32& target) {
            ...
            m_source        = createEmptyThalloImage(m_dims, ThalloImage::Type::FLOAT, 1, ThalloImage::Location::GPU, true);
            m_target        = createEmptyThalloImage(m_dims, ThalloImage::Type::FLOAT, 1, ThalloImage::Location::GPU, true);
            m_targetDU      = createEmptyThalloImage(m_dims, ThalloImage::Type::FLOAT, 1, ThalloImage::Location::GPU, true);
            m_targetDV      = createEmptyThalloImage(m_dims, ThalloImage::Type::FLOAT, 1, ThalloImage::Location::GPU, true);
            m_flowVectors   = createEmptyThalloImage(m_dims, ThalloImage::Type::FLOAT, 2, ThalloImage::Location::GPU, true);
            ...
            updateThalloImage(m_source, source);
            updateThalloImage(m_target, target);
            updateThalloImage(m_targetDU, targetDU);
            updateThalloImage(m_targetDV, targetDV);
            updateThalloImage(m_flowVectors, initFlowVectors);
		...
		void resetFlowVectors() {
            ...
            BaseImage<float2> initFlowVectors(m_dims[0], m_dims[1]);
			initFlowVectors.setPixels(make_float2(0.0f, 0.0f));
            m_flowVectors->update((void*)initFlowVectors.getData(), sizeof(float2)*m_dims[0] * m_dims[1], ThalloImage::Location::CPU);
		}
		...
		std::shared_ptr<ThalloImage>   m_source;
		std::shared_ptr<ThalloImage>	m_target;
		std::shared_ptr<ThalloImage>	m_targetDU;
		std::shared_ptr<ThalloImage>	m_targetDV;
		std::shared_ptr<ThalloImage>	m_flowVectors;	//unknowns
	};
