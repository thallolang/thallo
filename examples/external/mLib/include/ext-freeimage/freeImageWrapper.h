...
namespace ml {
	...
	public:
		...
		static void saveImage(const std::string &filename, const BaseImage<T>& image, bool debugPrint = false) {
            ...
            bool isTiff = filename.length() > 4 && filename.find(".tif") != std::string::npos;
			...
			if (filename.length() > 4 && filename.find(".jpg") != std::string::npos ||
                ...
                if (isTiff && numChannels == 1 && bytesPerChannel == 4) {
                    type = FIT_FLOAT;
                    bpp = 32;
                }


                FIBITMAP *dib = FreeImage_AllocateT(type, width, height, bpp);
				...
				if (numChannels == 1 && bytesPerChannel == 1) {
					...
					for (unsigned int y = 0; y < height; y++) {
                        ...
                        if (isTiff) {
                            ...
                            for (unsigned int x = 0; x < width; x++) {
                                float v;		convertToFLOAT(v, image(x, y));
                                bitsRowStartFloat[x] = v;
                            }
                        } else {
                            for (unsigned int x = 0; x < width; x++) {
