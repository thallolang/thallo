...
int main(int argc, char *argv[]) {
    ...
    params.thallofile = "spatially_varying_deconvolution.t";
    ...
    std::string prefix = "../data/sv_deconv/";
    ...
    BaseImage<float> init_diff = BaseImage<float>(result);
    ...
    for (uint y = 0; y < result.getHeight(); ++y) {
        for (uint x = 0; x < result.getWidth(); ++x) {
            ...
            float o = x0(x, y);
            float init_error = ((t - o)*(t - o));
            initialError += (double)init_error;
            init_diff.setPixel(x, y, init_error);
    ...
    FreeImageWrapper::saveImage("diff_init.tif", init_diff);
