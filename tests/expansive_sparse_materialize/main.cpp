...
double solveGraphLaplacian(int width, float* unknown, float* target, const int edgeCount, int* edgeStart, int* edgeEnd) {
    ...
    Thallo_Problem* problem = Thallo_ProblemDefine(state, "expansive_sparse_materialize.t", "gauss_newton");
...
int main(){
    ...
    float scratch[dim*channels];
    srand(0xDEADBEEF);
    for (int i = 0; i < dim*channels; ++i) {
        scratch[i] = (float)((double)rand() / (double)RAND_MAX);
    }
    float *target, *unknown;
    
    size_t fSize = dim*channels*sizeof(float);
    ...
    saveMonochromeImage("target.png", channels, dim, target);
    saveMonochromeImage("result.png", channels, dim, unknown);
    printf("\nminimal_fitting %g\n", cost);
